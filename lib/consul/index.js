...
/* istanbul ignore next */
exports.setToken = function(dc) {
  datacenter = dc;
};

var write_content_to_consul = function(key_name, content, cb) {
  logger.trace('Adding key %s, value:\n%s', key_name, content);
  consul.kv.set({'key': key_name, value: content !== null ? String(content) : null, token: token, dc: datacenter}, function(err) {
    if (err) {
...
var file_deleted = function(branch, file, cb) {
  ...
  consul.kv.del({'key': key_name, token: token, recurse: branch.expand_keys, dc: datacenter}, function(err) {
    /* istanbul ignore if */
    if (err) return cb('Failed to delete key ' + key_name + ' due to ' + err);
    cb();
  });
...
exports.getLastProcessedRef = function(branch, cb) {
  ...
  consul.kv.get({'key': key_name, token: token, dc: datacenter}, function(err, item) {
    /* istanbul ignore if */
