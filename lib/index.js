...
global.datacenter = process.env.CONSUL_DATACENTER || null;
...
for (var i=2; i<process.argv.length; ++i) {
  ...
  if(process.argv[i] === '-d' || process.argv[i] === '--datacenter') {
    if(i+1 >= process.argv.length) {
      logger.error("No datacenter value provided with --datacenter option");
      process.exit(8);
    }
    global.datacenter = process.argv[i+1];
  }
